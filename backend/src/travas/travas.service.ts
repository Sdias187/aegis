...
const SORT_COLUMNS: Record<string, string> = { nome: 'NOME', descricao: 'DESCRICAO', endpoint: 'ENDPOINT', metodo: 'METODO', ativo: 'ATIVO', NOME: 'NOME', DESCRICAO: 'DESCRICAO', ENDPOINT: 'ENDPOINT', METODO: 'METODO', ATIVO: 'ATIVO' };
...
export class TravasService {
  ...
  async list(params: PaginationParams): Promise<PaginatedResponse<Trava>> {
    ...
    if (params.search) {
      conditions.push(`(UPPER(NOME) LIKE UPPER('%' || :search || '%') OR UPPER(DESCRICAO) LIKE UPPER('%' || :search || '%') OR UPPER(ENDPOINT) LIKE UPPER('%' || :search || '%') OR UPPER(METODO) LIKE UPPER('%' || :search || '%'))`);
      binds.search = params.search;
    }
    for (const [key, column] of [['nome', 'NOME'], ['endpoint', 'ENDPOINT']] as const) {
      if (params[key]) { conditions.push(`UPPER(${column}) LIKE UPPER('%' || :${key} || '%')`); binds[key] = params[key]; }
    }
    ...
    return { data: dataResult.rows.map((row) => this.mapTrava(row)), pagination: { page: params.page, limit: params.limit, total, totalPages: Math.ceil(total / params.limit) } };
  }
