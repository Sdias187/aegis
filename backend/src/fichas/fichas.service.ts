...
import type { CreateFichaDto, Ficha, PaginatedResponse, PaginationParams, UpdateFichaDto } from '../common/interfaces';

const SORT_COLUMNS: Record<string, string> = {
  atendimentoPara: 'ATENDIMENTO_PARA', servico: 'SERVICO', ofertaServico: 'OFERTA_SERVICO',
  detalheFalha: 'DETALHE_FALHA', categoria: 'CATEGORIA', subcategoria: 'SUBCATEGORIA',
  ATENDIMENTO_PARA: 'ATENDIMENTO_PARA', SERVICO: 'SERVICO', OFERTA_SERVICO: 'OFERTA_SERVICO',
  DETALHE_FALHA: 'DETALHE_FALHA', CATEGORIA: 'CATEGORIA', SUBCATEGORIA: 'SUBCATEGORIA',
};
...
export class FichasService {
  ...
  async list(params: PaginationParams): Promise<PaginatedResponse<Ficha>> {
    ...
    const filters: Array<[keyof PaginationParams, string]> = [
      ['atendimentoPara', 'ATENDIMENTO_PARA'], ['servico', 'SERVICO'],
      ['ofertaServico', 'OFERTA_SERVICO'], ['detalheFalha', 'DETALHE_FALHA'],
    ];

    if (params.search) {
      conditions.push(`(UPPER(ATENDIMENTO_PARA) LIKE UPPER('%' || :search || '%') OR UPPER(SERVICO) LIKE UPPER('%' || :search || '%') OR UPPER(OFERTA_SERVICO) LIKE UPPER('%' || :search || '%') OR UPPER(DETALHE_FALHA) LIKE UPPER('%' || :search || '%'))`);
    ...
    return { data: dataResult.rows.map((row) => this.mapFicha(row)), pagination: { page: params.page, limit: params.limit, total, totalPages: Math.ceil(total / params.limit) } };
  }
  ...
  async create(data: CreateFichaDto): Promise<Ficha> {
    await this.db.executeQuery(`INSERT INTO AEGIS_FICHAS (ATENDIMENTO_PARA, SERVICO, OFERTA_SERVICO, DETALHE_FALHA, CATEGORIA, SUBCATEGORIA) VALUES (:atendimentoPara, :servico, :ofertaServico, :detalheFalha, :categoria, :subcategoria)`, this.toBinds(data));
    return { atendimentoPara: data.atendimentoPara, servico: data.servico, ofertaServico: data.ofertaServico, detalheFalha: data.detalheFalha, categoria: data.categoria?.trim() || undefined, subcategoria: data.subcategoria?.trim() || undefined };
  }
  ...
  private toBinds(data: CreateFichaDto): Record<string, unknown> {
    return { atendimentoPara: data.atendimentoPara, servico: data.servico, ofertaServico: data.ofertaServico ?? null, detalheFalha: data.detalheFalha ?? null, categoria: data.categoria?.trim() || null, subcategoria: data.subcategoria?.trim() || null };
  }

  private mapFicha(row: any): Ficha {
    return { id: row.ROW_ID, atendimentoPara: row.ATENDIMENTO_PARA, servico: row.SERVICO, ofertaServico: row.OFERTA_SERVICO ?? undefined, detalheFalha: row.DETALHE_FALHA ?? undefined, categoria: row.CATEGORIA ?? undefined, subcategoria: row.SUBCATEGORIA ?? undefined };
  }
}
